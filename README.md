...
Drug-Chartor是一个自动化的药品稳定性数据处理和趋势图生成工具。它可以从原始的Word/Excel文档中提取稳定性数据，并生成高质量静态趋势图。
...
3. **数据验证**：自动验证数据格式和完整性
4. **图表生成**：生成高质量静态趋势图（PNG），支持多批次对比和规格上下限展示
...
└── charts/             # 生成的交互式图表
    ├── 产品名_批号.png
    ├── 产品名_市场_条件_Nbatches.png
    └── chart_summary.txt
```

### 图表特性

- **静态高质量**：使用 Matplotlib 生成 PNG，适合报告归档
- **多批次对比**：同一产品、市场、条件的多个批次会自动合并到一张图
- **规格展示**：如表格中包含上下限，会在图中显示阈值线/区间
